...
class VisitorsController < ApplicationController
  ...
  def show
    ...
    if is_associated? # Do not show wallet
      ...
      show_error("#{GlobalConstants::DEBUG_MSG}-#{cn}: Not showing again wallet of existing visitor",
          "Our super monkeys detected that you already received a wallet with some Bitcoins before",
          true)
    else
      associate_wallet_and_visitor()
    end
  end 
  ...
  def is_associated?
    return ! (Visitor.where(ip: request.remote_ip)).empty?
  end

  def associate_wallet_and_visitor()
